...
public enum Server {
    ...
    private void start(int port) {
        ...
        try (ServerSocket serverSocket = new ServerSocket(port);) {
            ...
            logger.info(String.format(TextFX.colorize("Server is listening on port %s", Color.GREEN), port));
            isRunning = true;
            ...
            rooms.add(lobby);
            do {
                logger.info(TextFX.colorize("Waiting for next client", Color.BLUE));
                if (incoming_client != null) {
                    logger.info(TextFX.colorize("Client connected", Color.BLUE));
            ...
            logger.severe(TextFX.colorize("Error accepting connection", Color.RED));
            e.printStackTrace();
        } finally {
            logger.info(TextFX.colorize("Closing Server Socket", Color.GREEN));
        }
    }
    ...
    protected synchronized boolean joinRoom(String roomName, ServerThread client) {
        ...
        if (newRoom != null && roomName != null) {
            if (oldRoom != null && oldRoom != newRoom) {
                logger.info(String.format(TextFX.colorize("Client %s leaving old room %s", Color.PURPLE), client.getClientName(), oldRoom.getName()));
                oldRoom.removeClient(client);
            }
            logger.info(String.format(TextFX.colorize("Client %s joining new room %s", Color.PURPLE), client.getClientName(), newRoom.getName()));
    ...
    protected synchronized boolean createNewRoom(String roomName) {
        if (getRoom(roomName) != null) {
            ...
            logger.info(String.format(TextFX.colorize("Room %s already exists", Color.RED), roomName));
            return false;
        } else {
            ...
            rooms.add(room);
            logger.info(String.format(TextFX.colorize("Created new room %s", Color.YELLOW), roomName));
            return true;
        }
    }
    ...
    protected synchronized void removeRoom(Room r) {
        if (rooms.removeIf(room -> room == r)) {
            logger.info(String.format(TextFX.colorize("Removed empty room %s", Color.YELLOW), r.getName()));
        }
    }
    ...
    public static void main(String[] args) {
        Server.logger.info(TextFX.colorize("Starting server", Color.GREEN));
        ...
        Server.logger.info(TextFX.colorize("Server stopped", Color.GREEN));
    }
}